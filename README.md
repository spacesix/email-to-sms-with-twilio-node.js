...
My task is create a virtual sendmail user e2m, with email address: e2m@zen1.mydomain.com. Once I send a email with special format to e2m@zen1.mydomain.com. It will trigger a node.js script. this script will call a Twilio API to send out a SMS.
...
e2m:            "|node /home/e2s/e2s.js"                      ## add one line at the bottom. then save and exit vi.
[root@zen1 home]# newaliases
/etc/aliases: 79 aliases, longest 36 bytes, 867 bytes total   ## rebuild /etc/aliases
```

About will create a virual sendmail user. Once we send email to e2m@zen1.mydomain.com. sendmail will pipe the email content to node script: /home/e2s/e2s.js.

```bash
[root@zen1 home]# ln -s /home/e2m/e2m.js /etc/smrsh/e2m.js
[root@zen1 home]# ln -s /usr/bin/node /etc/smrsh/node
```
Make smrsh links, this will allow sendmail to call node and my script.
...
```
If you see your sendmail log has a similar line. then means you virual user and piping work properly.

Edit /home/e2m/e2m.config.json. Add below content
```jason
{
  "datemark": "20150320",
  "keys": [
...
Edit /home/e2m/e2m.js. Add below content. 
```javascript
/*
...
Then we send a email to local sendmail server for testing: The mobile number in here is on E.164 numbering format. 
Country Code	+ Area Code + Mobile number (without leading zero). If anythink ok, Mobile 61443555333 will receive a SMS "test sms 3"

```bash
[kevin@zen1 e2m]$ echo 'test sms 3' | mail -s '61443555333@USER1_1234565794fasld5' -v e2m@localhost
[kevin@zen1 e2m]$ tail e2m.log
  subject: '61443555333@USER1_1234565794fasld5',
  messageId: '201503230443.t2N4h88k026236@zen1.cardcall.com.au',
  priority: 'normal',
...
Mar 23 15:43:11 zen1 sendmail[26238]: t2N4h8XZ026237: to="|node /home/e2m/e2m.js", ctladdr=<e2m@zen1.cardcall.com.au> (8/0), delay=00:00:03, xdelay=00:00:03, mailer=prog, pri=30622, dsn=2.0.0, stat=Sent
```
